...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:php="http://php.net/xsl">
	...
	<xsl:template name="social">
		...
		<div class="social">
			...
			<script type="text/javascript"> window.___gcfg = {lang: 'ru'};(function() { var po = document.createElement('script'); po.type = 'text/javascript'; po.async = true; po.src
				='https://apis.google.com/js/plusone.js';var s = document.getElementsByTagName('script')[0];s.parentNode.insertBefore(po, s);})();
			</script>
		</div>
			...
			<xsl:when test="//section/node/node/add/id_parent[../../node[1]=$id] != ''">
				<xsl:call-template name="mites">
					...
					<xsl:with-param name="path">
						...
						">
						<xsl:value-of select="//section/node/node/add/name[../../node[1]=$id]" />
						&lt;/a>
	...
	<!--xsl:template name="mites"> <xsl:param name="parent"/> <xsl:param name="id"/> <xsl:param name="path"/> </xsl:template -->
	...
	<xsl:template match="list_articles">
		<div style="margin:10px;">
			<xsl:for-each select="articles/list">
				...
				<xsl:value-of select="//content/list_articles/count" />
			</xsl:for-each>
			<center>
	...
	<xsl:template match="news">
		<xsl:choose>
			...
			<xsl:when test="archive">
				<xsl:variable name="start">?section=3&amp;
				</xsl:variable>
				...
				<xsl:for-each select="archive[position()>3]">
					<div class="news_item1">
						...
						<xsl:value-of select="anons" disable-output-escaping="yes" />
						<a href="?section=3&amp;item={id}" class="news_button">
							подробнее
							<img src="images/news_button.gif" width="21" height="7" alt="Подробнее" />
						</a>
					</div>
					<xsl:if test="position()!=last()">
